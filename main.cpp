...
void human4(float tx, float ty)
{
    glPushMatrix();
    glTranslatef(tx, ty, 0);  // Apply translation to position the human

    // **Legs**
    glColor3ub(0, 0, 51);  // Dark blue color for pants
    glBegin(GL_QUADS);
    // Left leg
    glVertex3f(-0.5, 0.0, 0);
    glVertex3f(-0.5, -2.5, 0);
    glVertex3f(-0.2, -2.5, 0);
    ...
    // **Body**
    glColor3ub(0, 0, 255);  // Blue color for shirt
    glBegin(GL_QUADS);
    glVertex3f(-0.5, 0.0, 0);
    ...
    // **Head**
    glColor3ub(255, 224, 189);  // Light skin color
    glBegin(GL_POLYGON);
    float radius = 0.5;
    for (int i = 0; i <= 360; i += 10)
    {
        float theta = i * 3.14159 / 180;
        glVertex2f(radius * cos(theta), 2.5 + radius * sin(theta));
    }
    glEnd();
...
{
    glColor3ub(255, 255, 255); // White color for the football
    circleSolid(x, y, radius);
}
...
void display(void)
{
    ...
    human4(55.0, 10.0); // First human positioned slightly left
    human4(60.0, 10.0); // Second human positioned slightly right

    // Draw the football between the two humans
    drawFootball(58.5, -11.0, 1.5);
